...
async def get_contests(
    ...
    if organizer:
        ...
        query["organizer"] = {"$like": organizer}
    ...
    if deadline_from or deadline_to:
        ...
        # 数据库为 TEXT，统一按 YYYY-MM-DD 这类 ISO 字符串比较

        dr: dict = {}

        if deadline_from:

            dr["$gte"] = deadline_from
    ...
    cursor = await db.contests.find(query, skip=skip, limit=limit)
    ...
    for contest in contests:
        ...
        if is_public:
            ...
            contest.pop("entrant_url", None)

            contest.pop("teacher_url", None)

            contest.pop("default_url", None)

            contest.pop("contact_info", None)
